...
func init() {
	flag.StringVar(&hzNamespace, "namespace", "hazelcast-enterprise-operator-system", "The namespace to run e2e tests")
	flag.DurationVar(&interval, "interval", 1*time.Second, "The length of time between checks")
	flag.DurationVar(&timeout, "test-timeout", 5*time.Minute, "Timeout for test steps")
